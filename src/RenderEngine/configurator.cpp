...
// Checks the device for minimum game run requirements.
...
// Saves available devices in RenderEngine::physicalDevices global variable
void updateAvailableRenderDevices() {
    ...
    for (VkPhysicalDevice device : allDevices) {
        if (minimuPhysicalDeviceCheck(device)) {
            physicalDevices.push_back(device);
        }
    }
}

void createDevice(RenderSettings& settings) {
    ...
    vkCreateDevice(physicalDevices[settings.deviceId], &deviceCreateInfo, nullptr, &logicalDevice);
    currentPhysicalDeviceId = settings.deviceId;
}
...
void createSwapchain() {
    ...
    vkGetPhysicalDeviceSurfaceCapabilitiesKHR(physicalDevices[currentPhysicalDeviceId], surfaceKHR, &capabilities);

    uint32_t formatCount;
    vkGetPhysicalDeviceSurfaceFormatsKHR(physicalDevices[currentPhysicalDeviceId], surfaceKHR, &formatCount, nullptr);
    std::vector<VkSurfaceFormatKHR> formats(formatCount);
    vkGetPhysicalDeviceSurfaceFormatsKHR(physicalDevices[currentPhysicalDeviceId], surfaceKHR, &formatCount, formats.data());
...
VkFormat findSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features) {
    for (VkFormat format : candidates) {
        ...
        vkGetPhysicalDeviceFormatProperties(physicalDevices[currentPhysicalDeviceId], format, &props);
...
uint32_t findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
    ...
    vkGetPhysicalDeviceMemoryProperties(physicalDevices[currentPhysicalDeviceId], &memProperties);
...
void createTextureSampler() {
    ...
    VkPhysicalDeviceProperties properties{};
    vkGetPhysicalDeviceProperties(physicalDevices[currentPhysicalDeviceId], &properties);
