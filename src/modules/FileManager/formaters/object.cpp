...
Object loadObjectFile(std::string filename) {
    ...
    while (fileStream.getline(buff.data(), 256)) {
        ...
        if (substrings[0] == "v") {
            ...
            object.faces.push_back((std::array<std::array<int, 2>, 3>) {
                (std::array<int, 2>){ std::atoi(first[0].data()), std::atoi(first[1].data()) },
                (std::array<int, 2>){ std::atoi(second[0].data()), std::atoi(second[1].data()) },
                (std::array<int, 2>){ std::atoi(third[0].data()), std::atoi(third[1].data()) }
            });
        }
    }
